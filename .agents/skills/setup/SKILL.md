...
2. **Discordプラグインがインストールされているか**
   - `/mcp` でプラグイン一覧を確認
   - `plugin:discord:discord` がなければ：「Discordプラグインをインストールします」と伝え、`/plugin install discord@Codex-plugins-official` を案内
...
Codex --channels plugin:discord@Codex-plugins-official
...
**ここでPhase 1は終了。**
Phase 2は再起動後のセッションで自動的に引き継がれる（`.Codex/rules/setup-detect.md` がセットアップ未完了を検知し、`/setup-discord` を実行する）。
...
3. **起動フラグの確認**: `Codex --channels plugin:discord@Codex-plugins-official` で起動しているか
