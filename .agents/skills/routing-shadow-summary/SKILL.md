...
タナカ部門の shadow 計測ログ（`~/.Codex/stt-log/routing-shadow-YYYY-MM-DD.jsonl`）を
...
- 保存先: `~/.Codex/stt-log/routing-shadow-summary-YYYY-MM-DD.txt`
...
2. shadow ログを集計（`--by-session` 付き）:
   ...
   /Users/htk/work-manager/tools/say-menubar/.venv/bin/python \
     ...
     --date <YYYY-MM-DD> --by-session > ~/.Codex/stt-log/routing-shadow-summary-<YYYY-MM-DD>.txt
   ```
...
3. Discord `alert` チャンネルに、 保存ファイルの中身を整形して通知:
   ...
   <件数行 + hold 率 + diff 率>
   <セッション別上位 3 件>
   詳細: ~/.Codex/stt-log/routing-shadow-summary-YYYY-MM-DD.txt
   ```
...
- ログ source: `~/.Codex/stt-log/routing-shadow-YYYY-MM-DD.jsonl`
- サマリ保存: `~/.Codex/stt-log/routing-shadow-summary-YYYY-MM-DD.txt`
